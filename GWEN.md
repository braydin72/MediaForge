# MediaForge — Gwen Code Project Instructions

This file is read automatically at the start of every Gwen Code session in this repo. Follow these standing rules in addition to whatever the user asks for in their prompt.
...
Keep this update brief — a few bullet points, not a full essay. The goal is that a fresh Gwen Code session (or a human) can read it and know exactly where things stand without re-reading the whole git history.
