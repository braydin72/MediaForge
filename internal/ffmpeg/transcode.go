...
// If useCompletedDir=true, writes the output into a "completed/" subdirectory next to the source.
// Uses copy-then-delete instead of rename to support cross-filesystem moves.
// format should be "mkv" or "mp4"
func FinalizeTranscode(inputPath, tempPath, format string, replace bool, useCompletedDir bool) (finalPath string, err error) {
