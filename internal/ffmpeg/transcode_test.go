...
func TestBuildTempPath(t *testing.T) {
	...
	for _, tt := range tests {
		...
		dir := filepath.ToSlash(filepath.Dir(result)) // normalize separators for cross-platform comparison
...
func TestFinalizeTranscodeReplace(t *testing.T) {
	...
	// Finalize with replace=true, useCompletedDir=false (same directory)
	finalPath, err := FinalizeTranscode(originalPath, tempPath, "mkv", true, false)
	if err != nil {
		t.Fatalf("FinalizeTranscode failed: %v", err)
	}

	// Without useCompletedDir, output is in the same directory as the input
	expectedFinal := originalPath // mkv→mkv, same name
	if finalPath != expectedFinal {
		t.Errorf("expected final path %s to equal original path %s for mkv→mkv", finalPath, expectedFinal)
	}
...
func TestFinalizeTranscodeReplaceDifferentExt(t *testing.T) {
	...
	// Finalize with replace=true, useCompletedDir=false
	finalPath, err := FinalizeTranscode(originalPath, tempPath, "mkv", true, false)
	if err != nil {
		t.Fatalf("FinalizeTranscode failed: %v", err)
	}
	...
	// Without useCompletedDir, output is in the same directory, renamed to .mkv
...
func TestFinalizeTranscodeKeep(t *testing.T) {
	...
	finalPath, err := FinalizeTranscode(originalPath, tempPath, "mkv", false, false)
	if err != nil {
		t.Fatalf("FinalizeTranscode failed: %v", err)
	}
...
func TestFinalizeTranscodeCompletedDir(t *testing.T) {
	tmpDir := t.TempDir()
	...
	finalPath, err := FinalizeTranscode(originalPath, tempPath, "mkv", true, true)
	if err != nil {
		t.Fatalf("FinalizeTranscode failed: %v", err)
	}

	// With useCompletedDir, output goes into completed/ subdirectory
	...
	// completed/ directory should have been created
	if _, err := os.Stat(filepath.Join(tmpDir, "completed")); os.IsNotExist(err) {
		t.Error("completed/ directory was not created")
	}
}
